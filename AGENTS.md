...
- Docker mounts both snapshot directories and `test-results/` back to the host and runs as the host user, so generated PNGs and failure artifacts remain available without root-owned files. It does not mount host `node_modules`.
