...
- [x] Commit 3 completed on 2026-07-17: running tournaments admit signed-up entrants while the server-owned entry period is open; deterministic reconciliation queues or seats entrants at safe boundaries, grows and rebalances fields without a table cap, and renames/recreates final tables without changing active-hand history identity; waiting entrants participate in accounting, finish positions, and winner guards; registration actions, broadcasts, and canonical HTTP logs expose immediate, queued, and rejected outcomes. Focused late-registration, reconciliation, lifecycle, route, broadcast, and history-name tests pass, along with format, lint, typecheck, duplication and dependency checks, backend/frontend tests, E2E smoke, UI-catalog regression tests, the full E2E suite, and semantic review.
- [x] Commit 4 completed on 2026-07-17: the lobby now derives pre-start and late-registration actions from server state, identifies queued entrants as waiting for a table, shows queued-success feedback, and exposes the entry/rebuy periods through keyboard-accessible dark tooltips with SVG triggers. Frontend routing/component coverage and desktop/mobile UI-catalog cases cover eligible, queued, assigned, closed, rejected, pointer, and focus states. The existing 11-player stress lifecycle now starts with nine entrants, admits players 10 and 11 at levels 2 and 3, follows queued entrants through table assignment and movement, and includes them in play through the winner and OTS assertions. Full validation and the focused stress E2E pass; after final SVG-only refinements, focused frontend and MTT visual tests pass, and the user waived another full E2E rerun.
